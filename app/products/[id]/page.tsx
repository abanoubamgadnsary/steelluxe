...
import {
  Heart, ShoppingBag, Star, Shield, Truck, RefreshCw,
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchProduct = async () => {
      setLoading(true);
      try {
        const found = await getProductBySlug(params.id);
        ...
        if (found.sizes && found.sizes.length > 0) {
          const firstInStock = found.sizes.find(s => s.inStock);
          setSelSize(firstInStock?.label ?? '');
        }

        const relatedProducts = await getRelatedProducts(found.category, found.id);
  ...
  if (notFound || !product) {
    return (
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-24 text-center">
        <h1 className="font-display text-4xl text-charcoal-700 font-light mb-4">
          Product Not Found
        </h1>
        ...
        <Link
          href="/products"
          className="btn-gold px-8 py-4 rounded-full font-body font-medium text-sm inline-flex items-center gap-2"
        >
          Browse All Products
        </Link>
      </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
      ...
      <nav className="flex items-center gap-2 text-xs text-charcoal-400 mb-8 font-body">
        ...
        <Link
          ...
          className="hover:text-charcoal-700 transition-colors capitalize"
        >
          {product.category}
        </Link>
        <span>/</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 xl:gap-16">
        ...
        <div className="lg:sticky lg:top-24 lg:self-start space-y-6">
          ...
          <div className="flex items-center justify-between">
            <Link
              href={`/category/${product.category}`}
              className="text-xs tracking-widest uppercase text-gold-500 hover:text-gold-600 transition-colors font-body capitalize"
            >
              {product.category}
            </Link>
                ...
                <div className="flex gap-0.5">
                  {Array(5).fill(0).map((_, i) => (
                    <Star
                      key={i}
                      size={13}
                      className={i < Math.floor(product.rating) ? 'fill-gold-400 text-gold-400' : 'text-charcoal-200'}
                    />
                  ))}
                </div>
                <span className="text-xs text-charcoal-500 font-body">
          ...
          <div className="flex items-baseline gap-3">
            ...
            {product.discount && (
              <>
                <span className="text-lg text-charcoal-400 line-through font-body">
                  {formatPrice(product.price)}
                </span>
                <span className="badge-sale">Save {product.discount}%</span>
              </>
            )}
            {!product.discount && product.comparePrice && (
              <span className="text-lg text-charcoal-400 line-through font-body">
          ...
          <div className="flex items-center gap-2 text-sm font-body">
            <span className="font-medium text-charcoal-700">Availability:</span>
            {product.stock > 0
          ...
          {product.sizes && product.sizes.length > 0 && (
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <Link href="/size-guide" className="text-xs text-gold-500 hover:underline font-body">
                  Size Guide →
                </Link>
              </div>
            ...
            <button
              ...
              className={cn(
                ...
                wished
                  ...
                  : 'border-cream-300 text-charcoal-700 hover:border-gold-400 hover:text-gold-500'
              )}
            >
            ...
            <div className="text-sm text-charcoal-600 font-body leading-relaxed">
              ...
              {tab === 'shipping' && (
                <div className="space-y-2">
                  ...
                  <p className="text-gold-600 font-medium mt-3">
