...
export default function LoginPage() {
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true);
    try { await loginEmail(data.email, data.password); router.push('/'); }
  ...
  const handleGoogle = async () => {
    ...
    try { await loginGoogle(); router.push('/'); }
    catch { /* error already toasted */ }
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          ...
          <h1 className="font-display text-3xl text-charcoal-900 font-light mb-2">Welcome Back</h1>
          <p className="text-charcoal-400 font-body text-sm">Sign in to your account</p>
        </div>
        ...
        <div className="bg-white rounded-3xl shadow-card p-8">
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 py-3.5 border border-cream-300 rounded-xl text-sm font-body font-medium text-charcoal-700 hover:border-gold-300 hover:bg-cream-50 transition-all duration-200 mb-6 disabled:opacity-60"
          >
            {googleLoading ? (
              <span className="w-4 h-4 border-2 border-charcoal-300 border-t-charcoal-700 rounded-full animate-spin" />
            ) : (
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <label className="text-xs uppercase tracking-wider text-charcoal-500 font-body">Password</label>
                <Link href="/auth/forgot-password" className="text-xs text-gold-500 hover:underline font-body">Forgot password?</Link>
              </div>
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••"
                  className="w-full input-luxury rounded-xl pl-11 pr-12 py-3 text-sm font-body"
                />
                <button
                  type="button"
        ...
        <p className="text-center text-sm text-charcoal-500 mt-6 font-body">
          ...
          <Link href="/auth/register" className="text-gold-500 hover:underline font-medium">Create one</Link>
        </p>
      </div>
    </div>
  );
